...
@app.route('/api/map-wipe/<server>')
def api_map_wipe(server):
    if 'username' in session:
        exitcode, out, err = get_exitcode_stdout_stderr("./" + server + " map-wipe")
        conv = Ansi2HTMLConverter(inline=True)
        return conv.convert(out.decode('utf-8'))

@app.route('/api/full-wipe/<server>')
def api_full_wipe(server):
    if 'username' in session:
        exitcode, out, err = get_exitcode_stdout_stderr("./" + server + " full-wipe")
        conv = Ansi2HTMLConverter(inline=True)
        return conv.convert(out.decode('utf-8'))
...
@app.route('/api/update_oxide/<server>')
def api_update_oxide(server):
    if 'username' in session:
        exitcode, out, err = get_exitcode_stdout_stderr("./" + server + " mods-update")
        conv = Ansi2HTMLConverter(inline=True)
        return conv.convert(out.decode('utf-8'))
...
@app.route('/api/log/<server>')
def api_log(server):
    if 'username' in session:
        exitcode, out, err = get_exitcode_stdout_stderr("tail log/console/" + server + "-console.log -n 200")
        conv = Ansi2HTMLConverter(inline=True)
        log_text = out.decode('utf-8');
        log_text = log_text.replace('(Filename: ./Runtime/Export/Debug/Debug.bindings.h Line: 35)\r\n','')
        log_text = log_text.replace('(Filename: <f62ec4afc4f2422ea4f6f7abb79ec8bf> Line: 0)\r\n','')
        return conv.convert(log_text)


@app.route('/log/<server>')
def view_log(server):
    if 'username' not in session:
        flash("You do not have access to this section, please login!")
        return redirect(url_for('index'))

    return render_template('log.html', username=session['username'], servers=config['servers'], server=server)
